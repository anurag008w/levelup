...
export const memoryToolBatchSchema = z.object({
  actions: z.array(memoryToolActionSchema).min(1).max(20),
});
...
You can READ the memory, EDIT an entry's text, DELETE an entry, or PIN an entry into long-term memory (and UNPIN it).
...
- If the user asks to REMEMBER something new, just say it is now part of the conversation — do not invent an action.
- After executing, explain what you did in short Hinglish.`;

/** Keywords that route a user message to the memory tool decision hop. */
const MEMORY_QUERY_PATTERN =
  /(?:yaad rakho|yaad hai|yaad rakh|memory|memories|long[- ]?term|pehle bola|pehle bata|kaise yaad|kya yaad|yaad kar|yaad karo|yaad se|mat bhoolo|delete (?:kar|karo).*memory|edit.*memory|memory.*edit|memory.*delete|surakshit|bhaagne)/i;
