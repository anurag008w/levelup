  ...
  private async initPlatformNotifications(): Promise<void> {
    ...
    if (Capacitor.isNativePlatform()) {
      try {
        ...
        const handle = await LocalNotifications.addListener('localNotificationActionPerformed', (action) => { const extra = action.notification.extra; if (extra?.offlineMessage) this.injectMessageIntoChat(extra.offlineMessage); });
